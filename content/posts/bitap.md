...
func matchBitap(T, P string) int {
	...
	for j := range len(P) {
		// If we see c, then we are permitted to advance to state j+1 (assuming we were at state j before.)
		c := P[j]
		validMask[c] |= 1 << (j + 1)
	}
